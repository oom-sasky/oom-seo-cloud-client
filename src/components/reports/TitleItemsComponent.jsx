...
import TitleItemComponent from './TitleItemComponent';

const TitleItemsComponent = ({ title, reports, lengthType, loading, generating, generatedContent, handleGenerateClick, isMissingType }) => {
  ...
  const handleBulkGenerateClick = async () => {
    ...
    try {
      for (const report of selectedItems) {
        console.log(report);
        if (report) {
          await handleGenerateClick(report, 'title', report.pageUrl, report.pageContent);
        }
      }
    } catch (error) {
              ...
              {!isMissingType && (
                <>
                  <div className="oom-report-details_item-table_data-content_heading oom-column-30 title">
                    <p className="oom-report-details_item-table_data-content_heading_title">Title</p>
                  </div>
                  ...
                  <div className="oom-report-details_item-table_data-content_heading oom-column-10 title-length">
                    <p className="oom-report-details_item-table_data-content_heading_title">Title Length</p>
                  </div>
...
export default TitleItemsComponent;
